...
// APIs calling
let speaking_endpoint = '/api/speaking?perPage=1000';
let speaking_data;
let originalObj = null;
let formEvent = null;
get_speakings(speaking_endpoint);

async function get_speakings(endpoint){
    try {
        let headers = {
...
function render_speaking_data(data){
    speaking_data = data.data;
    let container = document.getElementById("speaking-services-container");
    if (speaking_data.length > 0) {
        container.innerHTML = '';
        speaking_data.forEach(obj => {
            let div = document.createElement("div");
            div.classList.add("coaching-services");
            let image = document.createElement("img");
            image.src = obj.image;
            image.alt = "speaking image";
            ...
            editButton.addEventListener("click", () => editSpeakings(obj));
            let deleteButton = document.createElement("button");
            deleteButton.innerText = "Delete";
            deleteButton.addEventListener("click", () => deleteSpeakings(obj));
...
consultingSearchForm.addEventListener("submit", (event)=>{
    ...
    speaking_endpoint = setParams(speaking_endpoint, 'search', data.search);
    get_speakings(speaking_endpoint);
})

function deleteSpeakings(obj) {
    originalObj = obj; 
    let modalId = 'deleteModal';
    let modal = document.getElementById(modalId);
    let form = modal.querySelector("form");
    form.querySelector("#message").innerText = 'Are you sure you want to delete this speaking?'
    formEvent = (event) => deleteSpeakingsForm(event);
...
async function deleteSpeakingsForm(event) {
    event.preventDefault();
    let form = event.target;
    let formData = new FormData(form);
    let data = formDataToObject(formData);
    ...
    try {
        ...
        beforeLoad(button);
        let response = await requestAPI(`/api/speaking/${originalObj.id}`, null, headers, 'DELETE');
...
add_speaking_button.addEventListener("click", () => {
    let modalId = 'addSpeakingModal';
    let modal = document.getElementById(modalId);
    let form = modal.querySelector("form");

    // Remove previous event listener before adding a new one

    formEvent = (event) => addSpeakingForm(event);
...
async function addSpeakingForm(event) {
    event.preventDefault();
    let form = event.target;
    let formData = new FormData(form);
    let data = formDataToObject(formData);
    ...
    try {
        ...
        beforeLoad(button);
        let response = await requestAPI('/api/speaking', formData, headers, 'POST');
        response.json().then(function(res) {
...
function editSpeakings(obj) {
    ...
    formEvent = (event) => updateSpeakingForm(event);
    form.addEventListener("submit", formEvent);
    ...
    let previewContainer = form.querySelector("#speaking-image-preview");
    previewContainer.innerHTML = returnImagePreviewHTML(obj?.image);
    ...
    modal.addEventListener('hidden.bs.modal', () => {
        ...
        modal.querySelector("button[type='submit']").querySelector(".btn-text").innerText = "Save";
...
async function updateSpeakingForm(event) {
    event.preventDefault();
    let form = event.target;
    let formData = new FormData(form);
    let data = formDataToObject(formData);
    ...
    try {
        ...
        beforeLoad(button);
        let response = await requestAPI(`/api/speaking/${originalObj.id}`, formData, headers, 'PATCH');
        let res = await response.json();
